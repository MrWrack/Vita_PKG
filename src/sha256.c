...
static void transform(MrwSha256 *ctx, const uint8_t data[]) {
    ...
    for(i=0;i<64;i++){t1=h+EP1(e)+CH(e,f,g)+k[i]+m[i];t2=EP0(a)+MAJ(a,b,c);
...
void mrw_sha256_update(MrwSha256 *ctx,const uint8_t *data,size_t len){
    for(size_t i=0;i<len;i++){ctx->data[ctx->datalen]=data[i];ctx->datalen++;
...
void mrw_sha256_final(MrwSha256 *ctx,uint8_t hash[32]){
    ...
    else{ctx->data[i++]=0x80;while(i<64)ctx->data[i++]=0;transform(ctx,ctx->data);for(i=0;i<56;i++)ctx->data[i]=0;}
    ...
    for(i=0;i<4;i++){hash[i]=(ctx->state[0]>>(24-i*8))&0xff;hash[i+4]=(ctx->state[1]>>(24-i*8))&0xff;
